...
def main():
    ...
    with st.sidebar:
        ...
        uploaded_files =  st.file_uploader("Upload your file",type=['docx'],accept_multiple_files=True)
        openai_api_key = openapi_key
        # openai_api_key = st.text_input("OpenAI API Key", key=openapi_key , type="password")
        process = st.button("Click to process")
    if process:
        ...
        st.session_state.conversation = get_conversation_chain(vetorstore,openai_api_key) #for openAI
...
def get_vectorstore(text_chunks):
    ...
    #Lecture31 model # embeddings = HuggingFaceEmbeddings(model_name="all-MiniLM-L6-v2")
    #a#embeddings = HuggingFaceEmbeddings(model_name="sentence-transformers/all-mpnet-base-v2")
    #b#embeddings = HuggingFaceEmbeddings(model_name="intfloat/e5-small-v2")
    #c#embeddings = HuggingFaceEmbeddings(model_name="albert-base-v2")
